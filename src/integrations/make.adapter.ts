...
export class MakeAdapter {
  get enabled() { return Boolean(env.MAKE_WEBHOOK_URL); }

  async publish(payload: PublishRequest): Promise<PublishResponse> {
    if (!env.MAKE_WEBHOOK_URL) {
      return { success: true, platform: payload.platform, warning: 'DRY_RUN: MAKE_WEBHOOK_URL not configured.', dryRun: true };
    }
    const result = await fetchJson<Record<string, unknown>>(
      env.MAKE_WEBHOOK_URL,
      {
        ...
        headers: {
          ...
          ...(env.MAKE_WEBHOOK_SECRET ? { 'X-Ghaith-Webhook-Secret': env.MAKE_WEBHOOK_SECRET } : {}),
        },
